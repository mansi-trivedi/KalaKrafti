...
const FeedbackCarousel: React.FC<CarouselProp> = (props) => {
  ...
  const next = () =>
    setCurr((curr) => (curr === customerFeedback.length - 1 ? 0 : curr + 1));
  ...
  return (
    <div className="relative overflow-hidden z-30">
      <div
        className={`flex transition-transform ease-out duration-700 h-[110vh]`}
        style={{ transform: `translateX(-${curr * 100}%)`, width: `100%` }}
      >
        {customerFeedback.map((slide, index) => (
          <div
            key={index}
            className="relative h-full text-center flex-shrink-0 flex flex-col justify-center items-center"
            style={{ width: `100%` }}
          >
            <div>
              <svg
            ...
            <h1 className="text-brick uppercase font-semibold text-4xl m-5 tracking-wider">
              They Said About Us
            </h1>
            <p className="text-brick w-[62vw] font-light tracking-wide mb-3">
              {slide.feedback}
            </p>
            ...
            <div className="text-center">
              <h1 className="text-brick font-semibold tracking-widest uppercase">
                {slide.name}
              </h1>
