...
const AllProduct = () => {
    ...
    return (
        <div className="flex py-16 bg-white flex-col lg:flex-row xl:flex-row 2xl:flex-row">
            <div className='px-2 lg:hidden xl:hidden 2xl:hidden text-brick flex-row flex' onClick={handleFilterModal}>
                <FaFilter size={20} />
                <p className='text-brick tracking-wider font-semibold px-2'>FILTER</p>
            </div>
            <CustomModal isOpen={filterModal} contentStyles={{ width: "90%", position: "relative" }}>
                <RxCross2 size={20} onClick={handleFilterModal} className='absolute right-3' />
                <Filter selectedCategories={selectedCategories} setSelectedCategories={setSelectedCategories} price={price} setPrice={setPrice} />
            </CustomModal>
                ...
                <div className='flex items-center p-2 space-x-2'>
                    {
                        selectedCategories.map((category, index) => (
                            <div key={index} className='flex justify-center items-center bg-brick rounded-full text-white text-sm font-semibold px-3 py-1'>
                                <span>{category}</span>
                                <RxCross2 className='ml-2' onClick={() => removeCategory(category)} />
                            </div>
                        ))
                    }
                    {
                        price.lable && <div className='flex justify-center items-center bg-brick rounded-full text-white text-sm font-semibold px-3 py-1'>
                    ...
                    }
                    {
                        ...
                        <button onClick={handleClearAll} className='border-2 border-brick rounded text-sm px-3 py-1 font-semibold'>
                            Clear All
                        </button>
                    }
                </div>
                    ...
                    {paginatedProducts.map((product, index) => (
                        ...
                        >
                            <div className="flex flex-col items-center pb-6">
                                ...
                                <p className="text-sm my-2 font-light text-center">{product.category}</p>
                                <p className="text-brick font-semibold text-center">{product.price}</p>
                            </div>
                        </div>
                    ))}
                </div>
